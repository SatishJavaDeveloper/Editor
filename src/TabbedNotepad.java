...
class Window_MenuItemsAction implements ActionListener
{
        ...
        public void actionPerformed(ActionEvent evt)
        {
            if(evt.getSource() instanceof JMenuItem)
            {
                ...
                switch(item)
                {
                    ...
                    case "Close" : File_Close_Action(-1);break;
    ...
    public void File_New_Action()
    {
    	...
    	//DefaultSyntaxKit.initKit();
         //final JEditorPane _textPane = new JEditorPane();
         _textPane.setFont(new Font("Calibri",Font.PLAIN,14));
        // _textPane.setEditable(true);
         
         //_textPane.setContentType("text/java");
         if(isDarkTheme){
         ...
         _textPane.setWrapStyleWord(true);
         ...
         JLabel lblTitle = new JLabel("Document "+count+" ");
        ...
        btnClose.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String temp = lblTitle.getText();
			
				if(_tabbedPane.indexOfTab(temp)==-1)
				{
					File_Close_Action(_tabbedPane.indexOfTab(temp+"*"));
				}
				else
				{
					File_Close_Action(_tabbedPane.indexOfTab(temp));
    ...
    public void File_Open_Action()
    {
         ...
         if (fd.getFiles()!=null)
         {
            ...
            for(File item : files)
            {
               ...
               JPanel pnlTab = new JPanel(new GridBagLayout());
               pnlTab.setOpaque(false);
               JLabel lblTitle = new JLabel(file);
               JButton btnClose = new JButton("x");
               ...
               _tabbedPane.setTabComponentAt(count, pnlTab);
               btnClose.addActionListener(new ActionListener() {
    ...
    // File -> Close action
    //********************************************************
    public void File_Close_Action(int selectedIndex)
    {
        if (_tabbedPane.getTabCount() > 0)
        {
        	int sel;
        	if(selectedIndex==-1)
        	{	
             sel = _tabbedPane.getSelectedIndex();
        	}
        	else
        	{
        		sel=selectedIndex;
    ...
    //********************************************************
          ...
          public static String getNodeTextContent(String nodetag)
          {
              ...
              try {
                  ...
                  File fXmlFile = new File("src/files/viewsfile.xml");
                  DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
                  DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
                  Document doc = dBuilder.parse(fXmlFile);
